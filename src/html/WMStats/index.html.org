<!DOCTYPE HTML>
<html lang="en">
  <meta charset="utf-8"></meta>
  <head>
    <title>WMStats</title>
    <style type="text/css" title="currentStyle">
        @import "@MOUNT@/css/jquery.dataTables.css";
        @import "@MOUNT@/css/ColVis.css";
        @import "@MOUNT@/css/jquery-ui-1.8.18.custom.css";
        @import "@MOUNT@/css/main_layout.css";
        @import "@MOUNT@/css/logdb.css";
        @import "@MOUNT@/css/font-awesome.min.css";
    </style>
  </head>
<body>
    <!-- content to be placed inside <body>…</body> -->
    <div id="monitor_title">
        <div id="wmstats_logo"></div>
        <div id="link_tabs"></div>
        <div id="clock"></div>
    </div>
    <div id="activeRequestPage">
        <!--
        <div id="summary_board"></div>
        <div id="message_board">
            <div id="agent_alert"></div>
            <div id="request_alert"></div>
        </div>
        //-->
        <div id="main_board">
            <fieldset id="filter_board"></fieldset>
            <fieldset id="filter_summary"></fieldset>
            <!-- <div id="filter_board"></div>
                 <div id="filter_summary"></div>
            -->
            <div id="status_board"></div>
            <div id="tab_board"></div>
            <div id='data_area'>
                <div id="data_board">
                    <div id="category_view">
                        <div id="category_bar"></div>
                        <div class="detail_data"></div>
                        <div class="summary_data"></div>
                    </div>
                    <div id="request_view">
                        <div id="view_switch_bar"></div>
                        <div id="category_title"></div>
                        <div class="detail_data hideDiv draggable"></div>
                        <div class="summary_data"></div>
                    </div>
                    <div id="job_view">
                        <div id="request_title"></div>
                        <div class="task_summary"></div>
                        <div class="summary_data"></div>
                        <div class="detail_data"></div>
                        <div id="acdc_submission" class="hideDiv draggable"></div>
                    </div>
                </div>
                <div id="loading_page" ></div>
            </div>
        </div>
        
    </div>
    <div id="requestAlertPage">
        <div class="summary_data"></div>
        <div id="alert_job_summary"></div>
        <div id="alert_job_detail"></div>
    </div>
    <div id="agentInfoPage">
        <div id="agent_summary"></div>
        <div id="agent_detail"></div>
    </div>
    <div id="logDBPage">
        <div id="logdb_summary" class="logdb_info warning"></div>
        <div id="logdb_detail"></div>
    </div>
    <div id="workloadSummaryPage">
        <div id="search_option_board"></div>
        <div id="search_result_board"></div>
    </div>
    <!--- templates this should be invisible -->
    <div id="templates">
        <div class="viewTemplate">
            <div class="detail_data hideDiv draggable"></div>
            <div class="summary_data"></div>
        </div>
        <div class="template stringMatch">\
            <input type="text" size="100" name="workloadSummarySearch" value=""></input>
        </div>
        <div class="template dateRange">
            <input type="text" name="dateRange1" class="dateValue"/> <span>through</span>
            <input type="text" name="dateRange2" class="dateValue"/>
        </div>
    </div>
</body>
<!-- Brings in all the dependencies needed for this couchapp -->
    <script src="@MOUNT@/js/loader.js" type='text/javascript'> </script>

    <!-- Globals is needed to be loaded before other library -->
    <script src="@MOUNT@/js/WMStats.Globals.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/WMStats.Utils.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/WMStats.Couch.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/WMStats.Ajax.js" type='text/javascript'> </script>
    
    <!-- importing order is critical -->
    <!-- data structure -->
    <script src="@MOUNT@/js/DataStruct/import.js" type='text/javascript'> </script>
    
    <!-- view model --->
    <script src="@MOUNT@/js/ViewModels/WMStats.ViewModel.js" type='text/javascript'> </script>
    
    <!-- view -->
    <script src="@MOUNT@/js/Views/import.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/Views/Controls/import.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/Views/Tables/import.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/Views/HTMLList/import.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/Views/Graphs/import.js" type='text/javascript'> </script>
    
    <!-- Overview.js will load javascripts corresponding to variant dynamically  -->
    <script src="@MOUNT@/js/Models/import.js" type='text/javascript'> </script>
    <script src="@MOUNT@/js/Controller/import.js" type='text/javascript'> </script>
    <script>
        WMStats.View.IndexHTML();
    </script>
</html>
